...
#+begin_quote
This file is retained as historical/literate reference material from the
original personal configuration. The supported cross-platform configuration now
loads from =init.el= and =lisp/eid-*.el=. See =README.md= and =INSTALL.md= for
the researcher-ready macOS/Linux setup.
#+end_quote
...
(evil-leader/set-key
  ...
  ;; orgmode
  "ob" 'org-archive-subtree-default
  "od" 'insert-todays-date
...
** org-agenda
   ...
   (setq org-agenda-span 'week)       ;; Show a whole week in the agenda
   (setq org-agenda-start-on-weekday 0) ;; Start the agenda from Sunday
   (setq org-agenda-start-day "-1d")  ;; Start the agenda from yesterday, so you can see upcoming tasks
   (setq org-agenda-skip-scheduled-if-deadline-is-shown nil) ;; Don't skip scheduled tasks
   (setq org-agenda-skip-deadline-prewarning-if-scheduled t) ;; Show scheduled tasks
...
(defun insert-todays-date ()
  "Insert today's date at the current cursor position."
  (interactive)
...
(setq org-publish-project-alist
      '(("org-website"
         :base-directory "~/repos/alkhaldieid.github.io/org/"
         :base-extension "org"
         :publishing-directory "~/repos/alkhaldieid.github.io/content/"
...
(use-package ox-hugo
  :ensure t
  :after ox)  ;; Load after org/ox (Org mode's exporter)
